...
_↓∘_ : ∀{B₀ B₁ ℓ C₀ C₁ ℓ′ D₀ D₁ ℓ″}
  {B : Category B₀ B₁ ℓ} {C : Category C₀ C₁ ℓ′} {D : Category D₀ D₁ ℓ″}
  {F G : Functor B C} → Nat F G → (H : Functor C D) → Nat (H ∘F F) (H ∘F G)
_↓∘_ {C = C} {D = D} {F = F} {G = G} η H = record
  { component = \r → fmap H (component η r)
  ; naturality = \{a} {b} {f} → let open ≈-Reasoning D in
    begin
      D [ fmap H (component η b) ∘ fmap (H ∘F F) f ] ↑⟨ covariant H ⟩
      fmap H (C [ component η b ∘ fmap F f ]) ↓⟨ ≈-cong H (naturality η) ⟩
      fmap H (C [ fmap G f ∘ component η a ]) ↓⟨ covariant H ⟩
      D [ fmap (H ∘F G) f ∘ fmap H (component η a) ]
    ∎
  }

_∘↓_ : ∀{B₀ B₁ ℓ C₀ C₁ ℓ′ D₀ D₁ ℓ″}
  {B : Category B₀ B₁ ℓ} {C : Category C₀ C₁ ℓ′} {D : Category D₀ D₁ ℓ″}
  {F G : Functor C D} → (H : Functor B C) → Nat F G → Nat (F ∘F H) (G ∘F H)
_∘↓_ {C = C} {D = D} {F = F} {G = G} H η = record
  { component = \r → component η (fobj H r)
  ; naturality = \{a} {b} {f} → let open ≈-Reasoning D in
    begin
      D [ component η (fobj H b) ∘ fmap (F ∘F H) f ] ↓⟨ naturality η ⟩
      D [ fmap (G ∘F H) f ∘ component η (fobj H a) ]
    ∎
  }
